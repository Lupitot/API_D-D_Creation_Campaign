<template>
  <form @submit.prevent="submitForm">
    <input type="text" placeholder="Nom de la campagne" v-model="name_of_campagne" />
    ...
    <input type="number" placeholder="Nombre de joueurs" v-model="number_of_player_recomended" />
    <input type="text" placeholder="Lieux importants" v-model="importants_place" />
...
export default {
  ...
  methods: {
    submitForm(event) {
        ...
        const idCamp = localStorage.getItem("currentCampId");
        let importants_place = Array.isArray(this.importants_place) ? this.importants_place : this.importants_place.split(',');
        modifCamp( idCamp ,this.name_of_campagne, this.name_of_world, this.difficulty, this.pitch, this.number_of_player_recomended, importants_place)
